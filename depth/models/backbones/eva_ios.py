...
class Eva(nn.Module):
    ...
    def _pos_embed(self, x) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        if self.dynamic_img_size:
            B, H, W, C = x.shape
            # print(int(math.sqrt(self.pos_embed.shape[1] - 1)))
            # hw = int(math.sqrt(self.pos_embed.shape[1] - 1))
            a1, a2 = self.patch_embed.patch_size
            k = float(a1) / a2
            o2 = int(math.sqrt((self.pos_embed.shape[1] - 1) / k))
            o1 = k * o2
            if self.pos_embed is not None:
                pos_embed = resample_abs_pos_embed(
                    self.pos_embed,
                    (H, W),
                    # old_size=self.patch_embed.patch_size,
                    # old_size=(int(14.*a1/a2), 14),
                    old_size=(int(o1), int(o2)),
                    # old_size=(7, 14),
                    num_prefix_tokens=self.num_prefix_tokens,
                )
            else:
    ...
    def forward(self, x):
        ...
        # x = x[:, :-1, :].view(B, int(np.ceil(H/self.patch_embed.patch_size[0])),
        #                       int(np.ceil(W/self.patch_embed.patch_size[1])), -1).permute(0, 3, 1, 2)
        # x = x[:, :-1, :].view(B, torch.ceil(H/self.patch_embed.patch_size[0]),
        #                       torch.ceil(W/self.patch_embed.patch_size[1]), -1).permute(0, 3, 1, 2)
        x = x[:, :-1, :].view(B, math.ceil(H/self.patch_embed.patch_size[0]),
                              math.ceil(W/self.patch_embed.patch_size[1]), -1).permute(0, 3, 1, 2)
...
def EVAIOS(pretrained=True,
    ...
    # )
    # model = _create_eva('eva02_large_patch14_224.mim_m38m',
    # model = _create_eva('eva02_tiny_patch14_224.mim_in22k',
    model = _create_eva('eva02_base_patch16_clip_224.merged2b',
    # model = _create_eva('eva02_small_patch14_224.mim_in22k',
